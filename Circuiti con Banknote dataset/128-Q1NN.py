...
def prePlotIris(data,x,y):
    X = data[data["class"] == 0]
    plt.scatter(X[x],X[y],color="red",marker="o",edgecolor='black', linewidth=lw,label="class 0")
    X = data[data["class"] == 1]
    plt.scatter(X[x],X[y],color="blue",marker="o",edgecolor='black', linewidth=lw,label="class 1")
...
q = QuantumRegister(7,"q") #qbit indice per i training
...
c4= ClassicalRegister(7,"c4") #for measure q0q1q2 indexes
...
for v in data.index:
    ...
    while not data.empty:
        ...
        for k in range(limit):
            ...
            encodeVector(circuit[p], trainingVector["f0":"f3"], j, [q[0], q[1], q[2],q[3],q[4],q[5],q[6], j[0],j[1],classe[0]], r[1], None)
            encodeIndex(circuit[p],k,q)
            encodeClasse(circuit[p],trainingVector["class"],classe)
        #fidelity
        circuit[p].h(prova[0])
...
for w in range(len_risultati):
    ...
    for k in range(m):
        ...
        if(value[10] == '1' and value[11] == '1' and value[13] == '0'):
...
with open(filename, 'w') as csvfile: 
    ...
    for i in range(len(ground_truth)):
        while j < len(prediction): 
            row = [['128',prediction[j],ground_truth[i]]]
