...
c4= ClassicalRegister(5,"c4") #for measure q0q1 indexes
...
for v in data.index:
    ...
    while not data.empty:
        ...
        for k in range(limit):
            ...
            controlled_gate = StatePreparation(trainingVector["f0":"f3"]).control(7)
            circuit[p].append(controlled_gate,[q[0],q[1],q[2],q[3],q[4],classe[0],classe[1],r2[0],r2[1]])
            encodeIndex(circuit[p],k,q)
            encodeClasse(circuit[p],trainingVector["class"],classe)
        #fidelity
        circuit[p].h(prova[0])
