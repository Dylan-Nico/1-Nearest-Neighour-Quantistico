...
q = QuantumRegister(4,"q") #qbit indice per i training
...
c4= ClassicalRegister(4,"c4") #for measure q0q1q2q3 indexes
...
for v in data.index:
    ...
    while not data.empty:
        ...
        for k in range(limit):
            ...
            encodeVector(circuit[p], trainingVector["f0":"f3"], j, [q[0], q[1], q[2], q[3], j[0],j[1],classe[1],classe[0]], r[1], None)
            encodeIndex(circuit[p],k,q)
            encodeClasse(circuit[p],trainingVector["class"],classe)
        #fidelity
        circuit[p].h(prova[0])
...
for w in range(len_risultati):
    ...
    for k in range(m):
        ...
        if(value[8] == '1' and value[9] == '1' and value[11] == '0'):
...
with open(filename, 'w') as csvfile: 
    ...
    for i in range(len(ground_truth)):
        while j < len(prediction): 
            row = [['16',prediction[j],ground_truth[i]]]
