...
c4= ClassicalRegister(3,"c4") #for measure q0q1q2 indexes
...
for v in data.index:
    ...
    while not data.empty:
        ...
        for k in range(limit):
            ...
            encodeVector(circuit[p], trainingVector["f0":"f3"], j, [q[0], q[1], q[2], j[0],j[1],classe[1],classe[0]], r[1], None)
            encodeIndex(circuit[p],k,q)
            encodeClasse(circuit[p],trainingVector["class"],classe)
        #fidelity
        circuit[p].h(prova[0])
        ...
        job = execute(circuit[p], simulator, shots=20000000)
        result = job.result()
...
for w in range(len_risultati):
    ...
    for k in range(m):
        ...
        if(value[7] == '1' and value[8] == '1' and value[10] == '0'):
...
with open(filename, 'w') as csvfile: 
    ...
    for i in range(len(ground_truth)):
        while j < len(prediction): 
            row = [['8',prediction[j],ground_truth[i]]]
